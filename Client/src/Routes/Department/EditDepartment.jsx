...
export default function EditDepartment({ department, onClose, onSuccess }) {
  const [form, setForm] = useState({
    dep_full: department.dep_full || "",
    dep_short: department.dep_short || "",
    deleteflag: department.deleteflag || "N",
    updatedby: "system", // เปลี่ยนเป็น updatedby
  });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.dep_full || !form.dep_short) {
      setError("กรุณากรอกข้อมูลที่จำเป็น (ชื่อเต็ม, ชื่อย่อ)");
      return;
    }
    ...
    try {
      ...
      // ส่ง PUT ไปที่ /department/:dep_code
      const res = await fetch(`${API_URL}/department/${department.dep_code}`, {
        method: "PUT",
        headers: {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 animate-in fade-in zoom-in-95">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200">
          <h2 className="text-lg font-semibold text-slate-800">
            Edit Department: <span className="text-red-600">{department.dep_code}</span>
          </h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          {/* 👉 แสดง dep_code เป็น Text ธรรมดา (ห้ามแก้) */}
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1">Department Code</label>
            <div className="w-full border border-slate-200 bg-slate-50 rounded px-3 py-1.5 text-sm text-slate-500">
              {department.dep_code}
            </div>
          </div>
