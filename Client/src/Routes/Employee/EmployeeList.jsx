...
function TypeBadge({ type }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-semibold border ${s.text}`}>
      {s.icon}
      {type}
    </span>
  );
...
const COLUMNS = [
    ...
    cell: (emp) => (
      <div className="flex items-center gap-3">
        <img
          ...
          onError={(e) => {
            e.target.src = `https://ui-avatars.com/api/?name=${emp.emp_name}&background=e2e8f0&color=475569`;
          }}
        />
        <div>
  ...
  {
    // ✅ Department column — แสดง dep_full + dep_short จาก join
    header: "Department",
...
function EmployeeExpandedContent(emp) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      {/* ✅ Department detail — แสดง dep_full, dep_code, dep_short */}
      <div>
        <p className="text-xs text-slate-500 font-semibold uppercase tracking-wider mb-2">
          Department
        </p>
        <p className="text-sm text-slate-800 font-medium">{emp.dep_full || "—"}</p>
        <div className="flex items-center gap-2 mt-1">
          <span className="text-[11px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded font-mono">
            {emp.dep_code}
          </span>
          {emp.dep_short && (
            <span className="text-[11px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded font-semibold border border-blue-100">
              {emp.dep_short}
            </span>
          )}
        </div>
        <p className="text-sm text-slate-500 mt-1">Type: {emp.emp_type}</p>
      </div>
      ...
      <div>
        ...
        <p className="text-sm text-slate-800">
          Created: {emp.createdat ? new Date(emp.createdat).toLocaleDateString("th-TH") : "—"}
        </p>
...
export default function EmployeeList() {
  ...
  // 👉 2. เพิ่ม State สำหรับ Edit Modal
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingEmployee, setEditingEmployee] = useState(null); // เก็บ object พนักงานที่จะแก้ไข
  const [refreshKey, setRefreshKey] = useState(0);

  const handleOpenModal = () => setIsModalOpen(true);

  // ใช้สำหรับทั้ง Create สำเร็จ และ Edit สำเร็จ
  const handleSuccess = () => {
...
const MENU_CONFIG = {
  ...
  // 👉 3. เปลี่ยน onEdit ให้เก็บข้อมูลพนักงานไว้ใน State
...
return (
  <>
    ...
    {/* 👉 4. Modal สำหรับแก้ไข (เปิดเมื่อมีการเซ็ต editingEmployee) */}
    {editingEmployee && (
      <EditEmployee 
