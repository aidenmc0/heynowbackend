...
export default function EditBooking({ booking, onClose, onSuccess }) {
  const [form, setForm] = useState({
    ...
    booking_adults: booking.booking_adults ?? 0,
    booking_children_free: booking.booking_children_free ?? 0,
    booking_children_addon: booking.booking_children_addon ?? 0,
    booking_pet_free: booking.booking_pet_free ?? 0,
    booking_pet_addon: booking.booking_pet_addon ?? 0,
    booking_checkin: booking.booking_checkin || "",
    booking_checkout: booking.booking_checkout || "",
    booking_slip: booking.booking_slip || null,
    confirm_id: booking.confirm_id || "",
  });
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl mx-4 animate-in fade-in zoom-in-95" style={{ maxHeight: "90vh", overflowY: "auto" }}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 sticky top-0 bg-white z-10">
          <h2 className="text-lg font-semibold text-slate-800">
            Edit Booking: <span className="text-red-600">{booking.booking_id}</span>
          </h2>
